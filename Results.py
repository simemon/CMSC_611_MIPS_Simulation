...
class Results:
    ...
    def print_row(self):
        ...
        result = result.format(text=self.instruction_text, fetch=self.fetch_stage, issue=self.issue_stage if self.issue_stage != 0 else "-", read = self.read_stage if self.read_stage != 0 else "-", executable=self.exec_stage if self.exec_stage != 0 else "-", write = self.write_stage if self.write_stage != 0 else "-", raw=self.raw_hazard, waw=self.waw_hazard, struct=self.struct_hazard)
        print(result)
